...
**Interview the Data** How big are you? Rows? Columns? List of column
names?
...
SF <- SF |> 
  janitor::clean_names() |> 
  mutate(
    call_date2 =  mdy(call_date), #make sure your data is actually in m - d - year format
...
```{r}
SF <- SF %>% 
  mutate(weekday = wday(call_date2, label=TRUE, abbr=FALSE)) #extracts weekday from date
```
