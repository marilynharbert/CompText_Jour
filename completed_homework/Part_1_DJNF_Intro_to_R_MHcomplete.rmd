...
#3: Number of rows cut with filter of greater that 5000 
  nrow(filter(MediaSalary, Salary>5000))
```
...
```{r}
#1: Build a table for NewYorkTimes employees   
NYT_Emp<- subset(MediaBucks, COMPANY=="NYT") 
...
#get max
#filter table for rows with salary of max value
...
biz_sal_sum <- summary(biz$Salary)
bloom_sal_sum <- summary(bloom$Salary)

# Bloomberg has a higher median salary than "Business"
...
```{r}
#1: Who is making $230,504 as a reporter???   
rich_reporter <- Reporters %>%
  filter(Salary==230504)

print(rich_reporter)
# A white man working as a senior reporter for ProPublica

#2: Make a table for editors, figure out medians.   

editors <- MediaBucks %>%
  filter(grepl("?editor", TITLE)) 
...
rich_editor <- editors %>%
  ...
  # channeling resentment

#4: Make a table for any position involving data  

data_people <- MediaBucks %>%
