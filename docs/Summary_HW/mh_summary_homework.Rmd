...
```{r}
meta_data_top_20_dv_windy_city <- read_excel("~/Code/CompText_Jour/docs/Summary_HW/windycity_dv_1_100.xls")
...
extracted_lines <- moley_index[15:215]
...
Merge metadata with index
```{r}
meta_data_top_100_dv_windy_city <- meta_data_top_20_dv_windy_city %>%
  rename(title = Title)

fullindex <- articles_df %>%
  left_join(meta_data_top_100_dv_windy_city)
...
fullindex <- fullindex %>%
  select(-c(date, publication, languageOfSummary, startPage))
...
year_plot <- ggplot(data=year_df, aes(year, n)) +
  geom_col() +
  labs(y = NULL, 
       x = "year") + 
  coord_flip() +
