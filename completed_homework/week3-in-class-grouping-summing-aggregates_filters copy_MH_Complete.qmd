...
baltcity_income <- read.csv(here("exercises", "assets/data/baltcity_income_clean.csv")) %>% 
  as.data.frame()
...
a100k_baltcity_income <- baltcity_income %>%
  #calculate median incomes for each neighborhood
  select(Neighborhood, x2020) %>%
  filter(x2020 >= 100000)
...
below_med <- baltcity_income %>%
  filter(x2020 < 49875) %>%
  count()

perc_below_med_ <- below_med$n/nrow(baltcity_income)


below_avg <- baltcity_income %>%
  filter(x2020 < 56311.21) %>%
  count()

perc_below_avg_ <- below_avg$n/nrow(baltcity_income)
